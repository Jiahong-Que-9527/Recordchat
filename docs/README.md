# RecordChat docs map

Start here only to find the right file. Execution order lives in
[project_plan.md](project_plan.md).

| File | Role |
|---|---|
| [../AGENTS.md](../AGENTS.md) | Agent entry + hard rules |
| [project_plan.md](project_plan.md) | **Where we are / what is next** |
| [agent_execution_playbook.md](agent_execution_playbook.md) | How to run any slice |
| [alh_execution_brief.md](alh_execution_brief.md) | v0.2.6 ALH slice (done — reference) |
| [v03_execution_brief.md](v03_execution_brief.md) | **v0.3.1 trial-user auth (current)** |
| [v03_ops.md](v03_ops.md) | Public hostname / Clerk / Tunnel / rollback |
| [architecture.md](architecture.md) | Runtime design |
| [../SPEC.md](../SPEC.md) | `/chat` contract (do not rename fields) |
| [v0.2_development_plan.md](v0.2_development_plan.md) | v0.2 historical breakdown |
| [roadmap.md](roadmap.md) | Milestone narrative |
| [v03_sketch.md](v03_sketch.md) | v0.3 remainder (3.2–3.7 still sketch) |
| [data_source_plan.md](data_source_plan.md) | Corpus + canonical versions |
| [adr/](adr/) | Decision records |
| [expert_session_script.md](expert_session_script.md) | External tester script + request-log freeze |

Current next work: **v0.3.1 trial-user auth**
([v03_execution_brief.md](v03_execution_brief.md)).
Do not start ONE Record Server persist or chat-body storage unless the user
overrides the plan.
